...
import static com.bn.ZDR.Particle.ParticleJiGuangDataConstant.*;
import static com.bn.ZDR.Util.Constant.SideLengh;

public class ParticleJiGuangSystem implements Comparable<ParticleJiGuangSystem>
{
	...
	private float xAngle=0;//此粒子系统的旋转角度
	private ParticleJiGuangForDraw fpfdJG;//粒子群的绘制者
	private boolean flag=true;//线程工作的标志位

	private float[] points;//粒子对应的所有顶点数据数组

	public ParticleJiGuangSystem(ParticleJiGuangForDraw fpfdJG, int count)
	{//构造器
		...
		this.fpfdJG=fpfdJG;//初始化粒子群的绘制者
		this.points=initPoints(count);//初始化粒子所对应的所有顶点数据数组
		fpfdJG.initVertexData(points);//调用初始化顶点坐标与纹理坐标数据的方法
		new Thread()
		{//创建粒子的更新线程
	...
	public float[] initPoints(int zcount)//初始化粒子所对应的所有顶点数据的方法
	{
		...
		{//循环遍历所有粒子
			...
			float px = (float)(Math.random()-0.5)*SideLengh;
			float py = 0.0f;
			float pz = 0.0f;
	...
	public void drawSelf(int texId)
	{
		...
		fpfdJG.drawSelf(texId,startColor,endColor,maxLifeSpan);//绘制粒子群
...
//					points[i*4+2]=pz;//粒子对应的第一个点的z坐标
		...
		{//加锁--防止在更新顶点坐标数据时，将顶点坐标数据送入渲染管线
			fpfdJG.updatVertexData(points);//更新顶点坐标数据缓冲的方法
		}
		count++;//下次激活粒子的位置
	}
	...
	{
		//根据摄像机位置计算火焰朝向
		float yspan=MatrixState3D.cameraPosition[1];
		float zspan=MatrixState3D.cameraPosition[2];
		...
		{
			xAngle=(float)Math.toDegrees(Math.atan(zspan/yspan));
		}
	}

	public int compareTo(ParticleJiGuangSystem another)
	{
		//重写的比较两个火焰离摄像机距离的方法
